...
CREATE PROCEDURE INPORT_XLS_PREVIA_APROVADA_ANGELONI AS
BEGIN
	...
	FROM OPENROWSET (
		...
		'Excel 12.0;Database=\\gru3wvamexdb1\Queries BI\2 - Monitorias\18 - Bradescard\BASES\ANGELONI_APROVADA.XLS;HDR=NO', 
	...
	FROM OPENROWSET (
		'Microsoft.ACE.OLEDB.12.0', 
		'Excel 12.0;Database=\\gru3wvamexdb1\Queries BI\2 - Monitorias\18 - Bradescard\BASES\ANGELONI_APROVADA.XLS;HDR=NO', 
		'SELECT * FROM [Indicadores Diarios$]')WHERE F1 LIKE 'Dados de:%')
	...
	while @CONT <= @INTERVALO_DIAS
		begin
			...
			DECLARE @DATA_PREVIA_APROVADA AS DATE
			SET @DATA_PREVIA_APROVADA = DATEADD(DD,@CONT-2,@DATA_INICIO)
			...
			'INSERT INTO #BASE_XLS
				SELECT	''112''													 AS CODIGO_PARCEIRO,
					...
					NULL														 AS ORG_LOGO,
					NULL														 AS MIGRACAO,
					''PRÉVIA APROVADA''											 AS DESCRITIVO,
					' + @BUSCADOR_CAMPO + '										 AS QUANTIDADE,
					' +  ''''+ CONVERT(VARCHAR,@DATA_PREVIA_APROVADA) + '''' + ' AS DATA_ACAO
	...
	INSERT VENDA_DIARIA_NOVOFRONT 
		  select tp.*, CONVERT(DATE,GETDATE ()) AS DATA_IMPORTACAO_ARQUIVO, 'XLS' ARQUIVO_IMPORTACAO from #BASE_XLS  as tp 
			where  not  EXISTS (select * from VENDA_DIARIA_NOVOFRONT as tf 
									...
									AND		(tf.DESCRITIVO		= 'PRÉVIA APROVADA' OR tf.DESCRITIVO = 'APROVADO CONSULTA PREVIA'))	
														 
END;


--executa procedure
EXEC  INPORT_XLS_PREVIA_APROVADA_ANGELONI
		/*
GO
