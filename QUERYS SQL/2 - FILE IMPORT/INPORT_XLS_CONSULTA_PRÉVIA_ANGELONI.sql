...
CREATE PROCEDURE INPORT_XLS_CONSULTA_PREVIA_ANGELONI AS
BEGIN
	...
	FROM OPENROWSET (
		...
		'Excel 12.0;Database=\\gru3wvamexdb1\Queries BI\2 - Monitorias\18 - Bradescard\BASES\ANGELONI_PREVIA.XLS;HDR=NO', 
	...
	while @CONT <= @INTERVALO_DIAS
		begin
			...
			DECLARE @DATA_CONSULTA_PREVIA AS DATE
			SET @DATA_CONSULTA_PREVIA = DATEADD(DD,@CONT-2,@DATA_INICIO)
			...
			'INSERT INTO #BASE_XLS
				SELECT	''112''													 AS CODIGO_PARCEIRO,
					...
					NULL														 AS ORG_LOGO,
					NULL														 AS MIGRACAO,
					''CONSULTA PRÉVIA''											 AS DESCRITIVO,
					' + @BUSCADOR_CAMPO + '										 AS QUANTIDADE,
					' +  ''''+ CONVERT(VARCHAR,@DATA_CONSULTA_PREVIA) + '''' + ' AS DATA_ACAO
	...
	INSERT VENDA_DIARIA_NOVOFRONT 
		  select tp.*, CONVERT(DATE,GETDATE ()) AS DATA_IMPORTACAO_ARQUIVO, 'XLS' ARQUIVO_IMPORTACAO from #BASE_XLS  as tp 
			where  not  EXISTS (select * from VENDA_DIARIA_NOVOFRONT as tf 
									...
									AND		(tf.DESCRITIVO		= 'CONSULTA PRÉVIA' OR tf.DESCRITIVO = 'ENVIADO CONSULTA PREVIA'))														 											 
END;

--executa procedure
EXEC  INPORT_XLS_CONSULTA_PREVIA_ANGELONI
		/*
GO
