...
CREATE PROCEDURE INPORT_XLS_PROPOSTA_ANGELONI AS
BEGIN
	...
	FROM OPENROWSET (
		...
		'Excel 12.0;Database=\\gru3wvamexdb1\Queries BI\2 - Monitorias\18 - Bradescard\BASES\ANGELONI_PROPOSTA.XLS;HDR=NO', 
	...
	while @CONT <= @INTERVALO_DIAS
		begin
			...
			DECLARE @DATA_PROPOSTA AS DATE
			SET @DATA_PROPOSTA = DATEADD(DD,@CONT-2,@DATA_INICIO)
			...
			'INSERT INTO #BASE_XLS
				SELECT	''112''													AS CODIGO_PARCEIRO,
					...
					NULL														AS ORG_LOGO,
					NULL														AS MIGRACAO,
					''PROPOSTA''												AS DESCRITIVO,
					' + @BUSCADOR_CAMPO + '										AS QUANTIDADE,
					' +  ''''+ CONVERT(VARCHAR,@DATA_PROPOSTA) + '''' + '		AS DATA_ACAO
	...
	INSERT VENDA_DIARIA_NOVOFRONT 
		  select tp.*, CONVERT(DATE,GETDATE ()) AS DATA_IMPORTACAO_ARQUIVO, 'XLS' ARQUIVO_IMPORTACAO from #BASE_XLS  as tp 
			where  not  EXISTS (select * from VENDA_DIARIA_NOVOFRONT as tf 
									...
									AND		(tf.DESCRITIVO		= 'PROPOSTA' OR tf.DESCRITIVO = 'EM DIGITAÇÃO'))
														 
END;


--executa procedure
EXEC  INPORT_XLS_PROPOSTA_ANGELONI
		/*
GO
